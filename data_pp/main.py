...
preprocess_types = {
    ...
    'adaptive_masking_equalized': [resize_image, adaptive_masking, histogram_equalization],
    'adaptive_masking_gaussian': [resize_image, adaptive_masking, histogram_equalization, gaussian_blur],
    'adaptive_masking_bilateral': [resize_image, adaptive_masking, histogram_equalization, bilateral_filter],
    'otsu': [resize_image, histogram_equalization, otsu_threshold], # Act as additional feature for model training
    'scharr': [resize_image, histogram_equalization, scharr_operator] # Act as additional feature for model training
}
...
def segregate_pneumonia_images(root_dir, output_dir):
    ...
    # Randomly move 8 images from the train/virus folder to the val/virus folder
    ...
    virus_images = [f for f in os.listdir(train_virus_folder) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
    
    # Randomly select 8 images
    selected_images = random.sample(virus_images, min(8, len(virus_images)))
        ...
        for c in classes:
            ...
            for key, value in preprocess_types.items():
                ...
                if isinstance(preprocessed_image, Image.Image):
                    ...
                    if preprocessed_image.dim() == 3 and preprocessed_image.shape[0] == 1:
                        preprocessed_image = preprocessed_image.squeeze(0)  # Remove channel dimension if single-channel
                    preprocessed_np = preprocessed_image.numpy().astype(np.uint8)
                else:
                    raise TypeError("Preprocessed image is neither a PIL Image nor a torch.Tensor")
